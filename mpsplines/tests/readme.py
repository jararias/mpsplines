...
def formatplot():
    ...
    ax.legend(bbox_to_anchor=(0.5, 0.05), loc='lower center', ncol=2,
...
def test_readme(n_known=8):

    x = np.linspace(0., 2*np.pi, n_known*60)
    y = (0.05 - np.sin(x))**2

    xi = np.reshape(x, (n_known, 60)).mean(axis=1)
    yi = np.reshape(y, (n_known, 60)).mean(axis=1)
    ...
    pl.figure('Plain mp-splines', figsize=(8, 3.5))
    background()
    mpi = MPI(yi=yi, xi=xi)
    pl.plot(x, mpi(x), 'r-', lw=1, label='mp-splines')
    formatplot()

    pl.figure('Periodic mp-splines', figsize=(8, 3.5))
    background()
    mpi = MPI(yi=yi, xi=xi, periodic=True)
    pl.plot(x, mpi(x), 'r-', lw=1, label='mp-splines periodic')
    formatplot()

    pl.figure('Periodic and constrained with min val mp-splines', figsize=(8, 3.5))
    background()
